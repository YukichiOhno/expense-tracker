...
router.post('/', authorizeToken, async (req, res) => {
    try {
        ...
        logger.error(err);
        return res.status(500).json({ message: 'an error occured while adding a budget' });
    }
});
...
router.get('/:user_number', authorizeToken, async (req, res) => {
    try {
        ...
        logger.error(err);
        return res.status(500).json({ message: 'an error occured while retrieving' });
    }
});

// update a budget for the current user
router.put('/:user_number/:budget_number', authorizeToken, async (req, res) => {
    try {
        const userInformationFromToken = req.user;
        const userNumber = userInformationFromToken.user_number;
        const parameterUserNumber = req.params.user_number;
        const budgetNumber = Number(req.params.budget_number);
        const newBudgetInformation = req.body;
        const userCurrencyCode = userInformationFromToken.curr_code;
        let userID;
        let selectQuery;
        let updateQuery;
        ...
        // validate incoming data
        if (!newBudgetInformation.amount || !newBudgetInformation.description || !newBudgetInformation.start_date || !newBudgetInformation.end_date) {
            logger.error('required budget information are missing');
            return res.status(400).json({ message: 'required budget information are missing' });
        }
        ...
        dollarToCurrency = resultQuery[0].dollar_to_curr;
        budgetInUSD = Number(newBudgetInformation.amount) / Number(dollarToCurrency);

        // update the budget entity
        updateQuery = "UPDATE budget SET bud_amt = ?, bud_desc = ?, bud_start_date = ?, bud_end_date = ? WHERE user_id = ? AND bud_seq = ?;";
        resultQuery = await executeWriteQuery(updateQuery, [
                budgetInUSD,
                newBudgetInformation.description.replace(/\s+/g, ' ').trim().toLowerCase(),
                newBudgetInformation.start_date.replace(/\s+/g, ' ').trim(),
                newBudgetInformation.end_date.replace(/\s+/g, ' ').trim(),
                userID,
                budgetNumber
        ]);

        logger.debug('successfully update the budget for the user');
        return res.status(200).json({ message: 'successfully update the budget for the user'});

    } catch (err) {
        logger.error('an error occured while updating a budget instance for the user');
        ...
        logger.error(err);
        return res.status(500).json({ message: 'an error occured while updating a budget instance for the user' });
    }
});
...
router.delete('/:user_number/:budget_number', authorizeToken, async (req, res) => {
    try {
        ...
        logger.error(err);
        return res.status(500).json({ message: 'an error occured while deleting an instance of budget for the user' });
    }
});
